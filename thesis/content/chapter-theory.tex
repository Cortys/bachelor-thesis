...
\begin{align*}
	...
	G :=&\ (X, E) \\
	\mathcal{C} :=&\ \text{Menge der maximalen Cliquen in $G$} \\
	...
	P(X = x) =&\ \frac{1}{Z} \prod_{c \in \mathcal{C}} \Phi_c(x_c) \text{, mit Normalisierungskonstante } Z := \sum_{x \in \mathcal{X}} \prod_{c \in \mathcal{C}} \Phi_c(x_c)
\end{align*}
...
\begin{tabular}{m{0.25\textwidth} m{0.75\textwidth}}
	...
	\end{align*}}
\end{tabular}\\
Gemäß dieser Definition, lässt sich die Erfüllbarkeit durch $\max_x P(X = x) > 0$ ausdrücken, d.~h.\ die Formel ist erfüllbar, wenn es eine Variablenbelegung mit Eintrittswahrscheinlichkeit~$> 0$ gibt.
Die Normalisierungskonstante $Z$ hat auf das Ergebnis keinen Einfluss und kann daher ignoriert werden.

Da sich SAT, wie soeben exemplarisch gezeigt, auf MRFs reduzieren lässt, ist das Finden einer Belegung der Zufallsvariablen ein NP-schweres Problem.
Allgemeine, exakte und effiziente Lösungsalgorithmen existieren daher nicht.
Durch Einschränkung der Struktur von $G$ und $\Phi$ oder durch das erlauben von Approximationen, lassen sich MRFs jedoch deutlich effizienter lösen.
