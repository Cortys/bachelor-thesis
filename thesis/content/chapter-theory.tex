...
\paragraph{Konzeptknoten (\textit{concepts})}
	...
	\begin{align*}
		\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/conceptNode.pdf}}}
...
\begin{align*}
	\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/relationNode.pdf}}}
	\quad\Leftrightarrow\quad
...
\begin{align*}
	\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/negationNode1.pdf}}}
	\quad\Leftrightarrow\quad
...
\begin{align*}
	\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/negationNode1.pdf}}}
	\quad\Leftrightarrow\quad
	\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/negationNode2.pdf}}}
\end{align*}
...
\begin{align*}
	\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/negationNode3.pdf}}}
	\quad\Leftrightarrow\quad
	&{\color{rot}\lnot\exists}\ a\ {\color{rot}\lnot\exists}\ b: P(a, b) \numberthis \\
	\quad\Leftrightarrow\quad
	&{\color{rot}\forall}\ a\ \exists\ b: P(a, b) \\[1ex]
	\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/negationNode4.pdf}}}
	\quad\Leftrightarrow\quad
...
\begin{align}
	\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/coreferenceEdge.pdf}}}
	\quad\Leftrightarrow\quad
...
\begin{align*}
	\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/coreferenceEdge.pdf}}}
	\quad\Leftrightarrow\quad
	\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/coreferenceEdgeAlternative.pdf}}}
\end{align*}
...
\begin{align*}
	...
	E =\ &\{ e: \mathit{coref}(e) \} \mathbin{\dot\cup} \{ e: \mathit{arg}(e) \} \mathbin{\dot\cup} \{ e: nest(e) \} \\ % chktex 35
	...
	neg(v) :\Leftrightarrow\ &\text{$v \in context$ ist ein Negationskontext} \displaybreak[0]\\
	\mathit{coref}(v_1, v_2) :\Leftrightarrow\ &concept(v_1) \land concept(v_2) \land (v_1, v_2) \in E \\
	\mathit{arg}(r, v) :\Leftrightarrow\ &relation(r) \land concept(v) \land ((r, v) \in E \lor (v, r) \in E) \\ % chktex 35
	nest(c, v) :\Leftrightarrow\ &context(c) \land (c, v) \in E \\
	E \supseteq\ &\{ (\top, v): v \in V \land \lnot\exists\, c \in V \setminus \{ \top \}: nest(c, v) \} \label{eq:theory:kgcrit0}\numberthis \\ % chktex 21
	& \lnot \exists\, c_1, c_2, a:nest(c_1, a) \land nest(c_2, a) \label{eq:theory:kgcrit1}\numberthis % chktex 21
\end{align*}
Zentral in dieser Definition ist das Kriterium~\eqref{eq:theory:kgcrit1}.
Es fordert, dass der Teilgraph $T = (V, \{ e: nest(e) \})$ zyklenfrei (i.~e.\ ein Wald) ist.
Durch~\eqref{eq:theory:kgcrit0} gilt darüber hinaus, dass $T$ ein Baum ist.
Hierfür wird der globale Kontext $\top$ eingeführt, der, in Anlehnung an Peirce, \textit{Sheet of Assertion} genannt wird.
$\top$ enthält alle Knoten, die keinen explizit dargestellten Elternkontext haben.
Die Wurzel von $T$ ist also $\top$.

Zusätzlich muss jeder Konzeptgraph $G$ die Eigenschaft dominierender Knoten $dom(G)$ erfüllen.
...
\end{align*}
Es gilt $a \leq b$, wenn $b$ ein Kontext ist, der $a$ enthält, oder wenn $a$ und $b$ den selben Elterkontext haben, oder wenn aufgrund der Transitivität $a \leq x \leq b$ gilt.
Das größte Element gemäß $\leq$ ist also immer $\top$.
Zusammenfassend beschreibt $a \leq b$, dass $a$ innerhalb der Box des Elternkontextes von $b$ liegt.
In \figreft{fig:theory:kgorder} wird dieser Zusammenhang veranschaulicht.
\begin{figure}[h]
	\centering
	\[\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/contextTreeExample1.pdf}}}
	\qquad
	\vcenter{\hbox{
		\begin{tikzpicture}[
			grow=down,
...
\begin{align*}
	dom(G) :\Leftrightarrow\ & \quad \forall\ r, v \in V: \mathit{arg}(r, v) \rightarrow r \leq v \numberthis \\ % chktex 35
...
Es ist nicht sinnvoll ist die Existenz eines Atoms auszudrücken, welches durch nicht existente Variablen parametrisiert ist.
...
\begin{figure}[h]
	\begin{align*}
		\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/dominatingNodeViolation1.pdf}}}
		\quad
		&\text{\color{rot}$\lightning\ \lnot dom(G)$, da $\mathit{arg}(P, b) \land \lnot (P \leq b)$.} \\ % chktex 35
		\vcenter{\hbox{\includegraphics[scale=0.23]{gfx/theory/dominatingNodeViolation2.pdf}}}
		\quad
...
\end{align*}
Es lässt sich zeigen, dass für die Verteilung $P'$ jedes MRFs mit positiver Dichte eine Potentialfunktion $\Phi$ existiert, sodass $P' = P$.
...
Zwei Klauseln mit denselben Variablen werden also durch zwei Potentiale, statt, wie in Cliquen-faktorisierten MRFs, durch eines, beschrieben.
