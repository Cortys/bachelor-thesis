...
\end{align*}
Es gilt $a \leq b$, wenn $b$ ein Kontext ist, der $a$ enthält, oder wenn $a$ und $b$ den selben Elternkontext haben, oder wenn aufgrund der Transitivität $a \leq x \leq b$ gilt.
...
\begin{enumerate}
	\item \textbf{Zufallsvariablen:}
		...
		Der Zufallsvektor des HL-MRFs ist also $X \cup Y$, wobei mit $\cup$ die Konkatenation der Vektoren gemeint ist.
...
\paragraph{BOCI}
Um effiziente Updates eines gegebenen Inferenzergebnisses für eine veränderte Eingabe $\mathcal{A'}$ zu ermöglichen, wurde das \textit{Budgeted Online Collective Inference} (BOCI) Verfahren entwickelt.
