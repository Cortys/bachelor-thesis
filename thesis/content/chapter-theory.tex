...
\begin{align*}
	0.5:&\ \lnot Friends(A, B) & (r_1) \\
	0.1:&\ Friends(A, B) \land Friends(B, C) \rightarrow Friends(A, C) & (r_2) \\
	0.8:&\ Interest(A, X) \land Interest(B, X) \rightarrow Friends(A, B) & (r_3) \\
...
\begin{align*}
	\mathbb{C} &= \left\{ % chktex 21
		\begin{aligned}
			& {\color{rot}Interest}(\text{``Alice''}, \text{``Reading''}),\ &&{\color{rot}Interest}(\text{``Dave''}, \text{``Reading''}), \\
			& {\color{rot}Interest}(\text{``Alice''}, \text{``Skiing''}),\ &&{\color{rot}Interest}(\text{``Charlie''}, \text{``Skiing''}), \\
			& {\color{rot}Interest}(\text{``Bob''}, \text{``Tennis''}),\ &&{\color{rot}Interest}(\text{``Charlie''}, \text{``Tennis''}), \\
			& {\color{gruen}Friends}(\text{``Alice''}, \text{``Bob''})
		\end{aligned}
	\right\} \\ % chktex 21
	\mathbb{O} &= \{{\color{blau}Friends}(x, y): x, y \in \{\text{``Alice''}, \text{``Bob''}, \text{``Charlie''}, \text{``Dave''}\}\} \setminus \mathbb{C} % chktex 21
\end{align*}
Die Interessen der Personen und die Freundschaft zwischen Bob und Alice aus $\mathbb{C}$ werden als wahr angenommen.
...
Personen mit gemeinsamen Interessen wurden aufgrund von $r_3$ als Freunde erkannt.
Wegen des Priors $r_1$ ist $r_3$ allein allerdings nicht hinreichend für eine $Friends$-Beziehung mit Wert $1$.
Hierfür ist die von $r_2$ modellierte Transitivitäteigenschaft notwendig;
in der Clique \textit{Alice-Bob-Charlie} haben sich die $Friends$-Beziehungen gegenseitig verstärkt.
