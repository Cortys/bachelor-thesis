...
\begin{enumerate}
	...
	\item \textbf{Prädikate:}
		...
		\centerline{\texttt{Person/1: UUID, Name/2: UUID $\times$ String, Female/1: UUID}}
	\item \textbf{Atome:}
		...
		\centerline{\texttt{Person(x), Name(x, ''Alice'')}} % chktex 32
	\item \textbf{Literale:}
		Ein Literal ist entweder ein Atom oder ein negiertes Atom. \\
		\centerline{\texttt{Name(x, ''Alice''), !Name(x, ''Bob'')}} % chktex 32
	\item \textbf{Regeln:}
		...
		\centerline{\texttt{0.65: Person(x) \& Name(x, ''Alice'') \rightarrow Female(x)}} % chktex 32, chktex 1
\end{enumerate}
...
Ein stark vereinfachtes PSL-Programm hierfür könnte wie folgt aussehen:
\begin{lstlisting}[mathescape=true]
(r1) 0.5: !Friends(A, B)
(r2) 0.6: Friends(A, B) & Friends(B, C) $\rightarrow$ Friends(A, C)
(r3) 0.8: Hobby(A, X) & Hobby(B, X) $\rightarrow$ Friends(A, B)
(r4)      Friends(A, B) $\rightarrow$ Friends(B, A).
\end{lstlisting}
In dieser Regelmenge sind vier Annahmen über das Verhalten der \texttt{Friends}-Relation kodiert:
\begin{enumerate}
	\item $r_1$ bildet ab, dass zwei Personen a~priori nicht befreundet sind.
		Eine solche PSL-Regel, die, in Ermanglung weiteren Wissens, das Nichtvorhandensein einer Relation postuliert, wird \textit{Prior} genannt.
		\textit{Priors} haben i.~d.~R. ein niedrigeres Gewicht, als die übrigen Regeln, damit sie bei der MAX-SAT-Inferenz leicht durch andere Regeln ``überstimmt'' werden können.
	\item $r_2$ bildet die Transitivität der \texttt{Friends}-Relation ab.
		Zwei Personen mit einem gemeinsamen Freund, sind evtl.\ befreundet.
	\item $r_3$ bildet ab, dass Personen mit einem gemeinsamen Hobby oftmals befreundet sind.
	\item $r_4$ bildet die Reflexivität der \texttt{Friends}-Relation ab.
		...
		d.~h. \texttt{Friends(A, B) = Friends(B, A)}.
\end{enumerate}
