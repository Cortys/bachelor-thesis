...
\begin{align*}
	...
	G :=&\ (V_X, E) \text{, mit einem Knoten $v_i \in V_X$ für jede Zufallsvariable $X_i \in X$} \\
	\mathcal{C} :=&\ \text{Menge der maximalen Cliquen in $G$} \\
	X_c :=&\ \text{Vektor der Zufallsvariablen in der Clique $c \in \mathcal{C}$} \\
	\Phi_c(x_c) :=&\ \text{Potential $\in \mathbb{R}^{+}_0$ der Werte $x_c$ von $X_c$} \propto P(X_c = x_c) \\
...
\end{align*}
Wie genau die Potentialfunktion $\Phi$ definiert ist, hängt primär von der modellierten Problemdomäne ab.
Um die obige Definition zu veranschaulichen folgt ein Beispiel.
...
\begin{tabular}{m{0.25\textwidth} m{0.75\textwidth}}
	\includegraphics[width=0.25\textwidth]{gfx/theory/mrfExample1.pdf}
	&
	{\begin{align*}
...
Gemäß dieser Definition, ist $x$ eine erfüllende Belegung gdw.\ $P(X = x) > 0$, d.~h.\ die Wahrscheinlichkeit, dass die Zufallsvariablen die Werte in $x$ annehmen, nicht null ist.
