...
\begin{enumerate}
	\item \textbf{Globale Markov-Eigenschaft:}
		\[
			\forall X_A, X_B, X_S \subseteq X: sep_{X_A, X_B}(X_S) \rightarrow (X_A\, \bot \, X_B\, |\, X_S)
		\]
	...
	\item \textbf{Lokale Markov-Eigenschaft:}
		\[
			\forall X_i \in X: X_i\, \bot \, (X \setminus \Gamma(X_i) \setminus \{X_i\})\, |\, \Gamma(X_i)
		\]
	...
	\item \textbf{Paarweise Markov-Eigenschaft:}
		\[
			\forall X_i, X_j \in X: \{X_i, X_j \} \notin E \rightarrow (X_i\, \bot \, X_j\, |\, X \setminus \{ X_i, X_j \})
		\]
...
Um die obigen Definitionen zu veranschaulichen folgt ein Beispiel.
...
\begin{enumerate}
	...
	\item
		...
		d.~h.\ jede Zufallsvariable in $X$ beschreibt die Wahrscheinlichkeit, dass eine aussagenlogische Variable wahr ist.
	\item
...
Aufgrund der Einschränkung HL-MRFs auf aussagenlogische Ausdrücke, wird im Folgenden die Graphterminologie fallen gelassen und stattdessen die entsprechende aussagenlogische Terminologie verwendet.
Ein HL-MRF wird nun als Repräsentation einer KNF-Formel $C_1 \land \dots \land C_m$ interpretiert.
Jede Disjunktionsklausel $C_j \in C$ wird durch eine Menge positiver Atome $I^{+}_j \subseteq X$ und eine Menge negativer Atome $I^{-}_j \subseteq X$ beschrieben.
\[
	C_j = \left(\bigvee_{x \in I^{+}_j} x\right) \lor \left(\bigvee_{x \in I^{-}_j} \lnot x\right)
\]
...
HL-MRFs benutzen hierfür die sog.\ Łukasiewicz Logik, eine Erweiterung der booleschen Logik.
Die Łukasiewicz Operatoren verhalten sich für die Extrema $0$ und $1$ so, wie die booleschen Operatoren, sind aber ebenfalls für alle dazwischen liegenden Eingabewerte definiert.
\begin{align}
