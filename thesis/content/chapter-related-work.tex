...
Basierend auf den Ideen der in~\ref{sec:related:kr:history} vorgestellten Wissensgraphen, stellte Google 2012 eine eigene, ebenfalls \textit{knowledge graph} genannte, Wissensgraphtechnologie vor.
...
\begin{figure}
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Popularität des Begriffs ``knowledge graph'' (Quelle: Google Trends)}\label{fig:related:kgTrend}
\end{figure}
...
\begin{enumerate}
	...
	\item \textbf{Part-of-speech Tagging:}
		...
		\begin{align*}
			...
			\Rightarrow
			(&\text{adverb}, \\
			 &\text{personal pronoun}, \\
			 &\text{present tense first-person singular verb}, \\
			 &\text{present participle verb}, \\
			 &\text{personal pronoun}, \\
			 &\text{sentence terminator})
		\end{align*}
	\item \textbf{Named Entity Recognition:}
		Klassifizierung von Tokens in Kategorien wie z.~B. Person, Ort oder Zeitpunkt.
		\[
			(\underbrace{\text{Today}}_{date}, \text{I}, \text{'m}, \text{testing}, \text{myself}, \text{.})
		\]
	...
	\item \textbf{Dependency Parsing:}
		...
		\begin{align*}
			(\text{Today}, \text{I}, \text{'m}, \text{testing}, \text{myself}, \text{.})
			\Rightarrow\ %
			& \text{testing} \xrightarrow{\text{subject}} \text{I} \\
			& \text{testing} \xrightarrow{\text{object}} \text{myself} \\
			& \text{testing} \xrightarrow{\text{auxiliary}} \text{'m} \\
			& \text{testing} \xrightarrow{\text{adverbial modifier}} \text{Today}
		\end{align*}
\end{enumerate}
...
Ein solches ist z.~B. das Stanford~CoreNLP Projekt~\cite{Manning2014}\cite{CoreNLP}, welches u.~a. Module für alle der soeben vorgestellten Verarbeitungsschritte beinhaltet.
Ein weiteres ähnliches Projekt ist Apache~OpenNLP~\cite{OpenNLP};\@
...
Ein Beispiel hierfür ist der Path Ranking Algorithmus (PRA)~\cite{Lao2011}. PRA ermittelt Relationen durch zufälliges Durchwandern des Graphen.
Um die Stärken der Latent Feature und Graph Feature Modelle zu kombinieren, wurden Hybrid-Modelle, wie z.~B. das Additive Relational Effects (ARE) Verfahren~\cite{Nickel2014}, entwickelt, welches die Konfidenzen von RESCAL und PRA addiert.
...
Wählt man ein Kalkül in dem die Atome stetig, also aus $[0, 1]$ statt aus $\{ 0, 1 \}$ sind, und Formeln zudem ausschließlich Disjunktionen und Negationen gemäß Łukasiewicz S-Norm enthalten, erhält man ein sog.\ Hinge-Loss-MRF~\cite{Bach2013}\cite{Bach2015}, da die Loss-Funktion der Disjunktion in dieser S-Norm ein Hinge-Loss ist.

Ein konkretes Kalkül, welches sich zur Spezifikation von HL-MRFs eignet, ist die Probabilistic Soft Logic (PSL)~\cite{Broecheler2010}\cite{Bach2015}.
MAX-SAT lässt sich für solche HL-MRFs effizient und parallelisierbar mit dem konvexen Alternating Direction Method of Multipliers (ADMM) Optimierungsverfahren~\cite{Boyd2011} lösen.
...
Um dieses Problem zu lösen, wurde das Budgeted Online Collective Inference (BOCI) Verfahren~\cite{Pujara2015} entwickelt.
