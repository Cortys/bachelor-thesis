		...
		\begin{enumerate}[label*={\arabic*}]
			\item Konzepte, die eine ungerade Anzahl von $neg$-Beziehungen zu Negations-Token haben, werden von einem negativen Aktualkontext umschlossen.
				\begin{align*}
					...
					\rightarrow
					\vcenter{\hbox{\includegraphics[scale=0.5]{gfx/text2kg/negContext1.pdf}}}
				\end{align*}
				...
				\begin{enumerate}[label*={.\arabic*}]
					\item Falls vor der Korrektur $y \leq x$ gilt, wird $x$ in den Kontext von $y$ bewegt.
						\begin{align*}
							\vcenter{\hbox{\includegraphics[scale=0.5]{gfx/text2kg/negContext1.pdf}}}
							\rightarrow
							\vcenter{\hbox{\includegraphics[scale=0.5]{gfx/text2kg/negContext2.pdf}}}
						\end{align*}
					...
					\begin{align*}
						\vcenter{\hbox{\includegraphics[scale=0.5]{gfx/text2kg/negContext2.pdf}}}
						\rightarrow
						\vcenter{\hbox{\includegraphics[scale=0.5]{gfx/text2kg/negContext3.pdf}}}
					\end{align*}
				\end{enumerate}
		\end{enumerate}
\end{enumerate}
...
\begin{enumerate}
	\item \textbf{Latente Konzepte:}
		...
		\begin{align*}
			\includegraphics[scale=0.5]{gfx/text2kg/latentGraph.pdf}
		\end{align*}
	\item \textbf{Nachrichtenmetadaten:}
		Damit der Konjunktionsgraph alle Informationen der einzufügenden Nachricht enthält, müssen neben dem Extraktionsgraph, der lediglich den Inhalt der Nachricht beschreibt, auch der Sender $S$ und Empfänger $R$ eingefügt werden.
		Dies geschieht durch folgende Erweiterung des Extraktionsgraphen $E$:
		\begin{align*}
			\includegraphics[scale=0.5]{gfx/text2kg/messageGraph.pdf}
		\end{align*}
...
\begin{itemize}
	...
	\begin{align*}
		1:\ & label(A, L_A) \land label(B, L_B) \land similar(L_A, L_B)  & \rulemark{rule:text2kg:inst:sim} \\
			...
			& \land A \neq B \land \lnot relation(A) \rightarrow inst(A, B) \\[1ex]
		0.1:\ & \lnot inst(A, B) & \rulemark{rule:text2kg:inst:prior} \displaybreak[0]\\[1ex]
		\infty:\ & inst(A, B) \land inst(B, C) \rightarrow inst(A, C) & \rulemark{rule:text2kg:inst:trans} \\[1ex]
...
Die verwendeten domänenspezifischen Regeln führen zusätzlich zu den \pslPredCount\ allgemeinen PSL-Prädikaten ein, die zur Repräsentation von Zwischenergebnissen benutzt werden.
...
\begin{align*}
	1:\ & context(ctx) \land patient(contain, ctx) \land agent(contain, msg) & \rulemark{rule:text2kg:message} \\
		& \land inst(\texttt{''concept\_contain''}, contain) \rightarrow message(ctx, msg) \displaybreak[0] \\[1ex]
	1:\ & message(ctx, msg) \land patient(send, msg) \land agent(send, sender) & \rulemark{rule:text2kg:sender} \\
		& \land inst(\texttt{''concept\_send''}, send) \rightarrow message(ctx, sender) \displaybreak[0] \\[1ex]
	1:\ & message(ctx, msg) \land patient(receive, msg) \land agent(receive, receiver) & \rulemark{rule:text2kg:receiver} \\
		& \land inst(\texttt{''concept\_receive''}, receive) \rightarrow message(ctx, receiver) \displaybreak[0] \\[1ex]
	1:\ & nest_{indirect}(ctx, I) \land inst(\texttt{''concept\_I''}, I) & \rulemark{rule:text2kg:i} \\
		& \land sender(ctx, sender) \rightarrow inst(sender, I) \displaybreak[0] \\[1ex]
	1:\ & nest_{indirect}(ctx, you) \land inst(\texttt{''concept\_you''}, you) & \rulemark{rule:text2kg:you} \\
		& \land receiver(ctx, receiver) \rightarrow inst(receiver, you)
\end{align*}
