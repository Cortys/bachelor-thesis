	...
	\item \textbf{Negative Aktualkontexte:}
		...
		\begin{enumerate}[label*={\arabic*}]
			\item Konzepte, die eine ungerade Anzahl von $neg$-Beziehungen zu Negations-Token haben, werden von einem negativen Aktualkontext umschlossen.
				...
				\end{align*}
			\item Falls im vorherigen Schritt zwei Konzepte negiert wurden, die miteinander in Beziehung stehen, ist nun die Eigenschaft dominierender Knoten verletzt.
				Außerdem gibt es potentiell $agent$- oder $patient$-Kanten, die in einen der Negationskontexe hineinlaufen, was gemäß der vorigen Überlegungen unzulässig ist.
				Für jede Relation $\color{rot}p(x, y)$, die aus einer dieser beiden Gründe, unzulässig ist, werden nun Korrekturen vorgenommen, sodass hinterher $x \leq y$ gilt und somit keine Regel-Verletzungen mehr vorliegen:
				\begin{enumerate}[label*={.\arabic*}]
