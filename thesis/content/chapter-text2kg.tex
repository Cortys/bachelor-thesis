...
Sie werden auf zwei Wegen aus den Annotationen extrahiert.
\begin{itemize}
...
\begin{itemize}
	...
	\item \textbf{Attribute, Adverbien und Appositionen:}
		Ähnlich wie Kopulae drücken diese drei grammatikalischen Beziehungen ein $inst$-ähnliches Verhältnis aus.
		\begin{align*}
			\text{Bo}\tikzmark{bob2}\text{b, a profes}\tikzmark{professional}\text{sional athl}\tikzmark{athlete}\text{ete, ru}\tikzmark{runs}\text{ns fa}\tikzmark{fast}\text{st.}
			\begin{tikzpicture}[overlay,remember picture]
				...
				\draw[->,rot,shorten >=3pt,shorten <=3pt] (fast.center) to[bend left] node[below] {\color{rot}$\scriptstyle inst$} (runs.center);
			\end{tikzpicture}
		\end{align*}
		Die Transformationsregel ist:
		\[amod(x, y) \lor advmod(x, y) \lor appos(x, y) \rightarrow inst(y, x)\]
\end{itemize}
...
Das Ergebnis ist nun beinahe ein Konzeptgraph.
...
Um dominierende Knoten zu erhalten, können die koreferenten Konzepte also zu einem Konzeptknoten zusammengefasst werden.
...
\paragraph{Kontexte}
Um einen möglichst kompakten Konzeptgraph zu erhalten, werden im letzten Schritt geschachtelte Kontexte vereinfacht.
