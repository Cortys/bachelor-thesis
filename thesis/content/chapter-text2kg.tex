...
Der Schwellwert wurde manuell gewählt~\tref{sec:evaluation}.
...
\begin{itemize}
	...
	\begin{align*}
		0.2:\ & label(A, L_A) \land label(B, L_B) \land similar(L_A, L_B)  & \rulemark{rule:text2kg:inst:sim} \\
			...
			& \land named(A) \land named(B) \rightarrow inst(A, B) \\[1ex]
	...
	\item \textbf{Geschlossene Prädikate:}}
		\infty:\ & \lnot named(A) & \rulemark{rule:text2kg:closed:named} \\[1ex]
		\infty:\ & \lnot relation(A) & \rulemark{rule:text2kg:closed:relation} \\[1ex]
		\infty:\ & \lnot nest(A) & \rulemark{rule:text2kg:closed:nest} \\[1ex]
...
\begin{align*}
	...
	1:\ & nest_{indirect}(ctx, you) \land inst(\texttt{''concept\_you''}, you) & \rulemark{rule:text2kg:you} \\
		& \land {\color{blau}receiver}(ctx, receiver) \rightarrow inst(receiver, you) \\[1ex]
