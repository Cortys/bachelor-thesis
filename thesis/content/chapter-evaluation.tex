...
\begin{figure}[h]
	...
	\begin{tikzpicture}
		...
		\begin{axis}[
			...
			\addplot+[no markers,style=thick,restrict x to domain=1:24] table [x expr=\coordindex+1, y=time]{\evaluationPerfData}
				node[above,pos=0.3]{\texttt{bin1/300249}};
			\addplot+[no markers,style=thick,restrict x to domain=25:32] table [x expr=\coordindex+1, y=time]{\evaluationPerfData}
				node[above=5pt,pos=0.5]{\texttt{bin1/363227}};
			\addplot+[no markers,style=thick,restrict x to domain=33:57] table [x expr=\coordindex+1, y=time]{\evaluationPerfData}
				node[above=10pt,pos=0.5]{\texttt{bin5/350143}};
		\end{axis}
	\end{tikzpicture}
	\caption{Inferenzdauer beim sequentiellen Einfügen der Testnachrichten}\label{fig:evaluation:perf}
\end{figure}
...
Dies lässt sich dadurch erklären, dass neue Threads i.~d.~R. neue Konzepte einführen, die mit den bereits vorhandenen Konzepten in Relation gebracht werden müssen.
Nachfolgende Nachrichten desselben Threads führen weniger neue Konzepte ein und bewirken daher keinen deutlichen Anstieg der Inferenzdauer.
